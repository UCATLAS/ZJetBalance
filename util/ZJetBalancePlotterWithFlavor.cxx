...
int main(int argc, char* argv[]) {
  ...
  vector<string> flavors = {"", "_b", "_c", "_l"};
  ...
  for (unsigned int iFlavor=0; iFlavor < flavors.size(); ++iFlavor){
    for (int iEtaBin=1; iEtaBin<netabins+1;++iEtaBin) {
      TH1F *h_mean  = new TH1F(Form("mean_pt_eta%d%s", iEtaBin, flavors.at(iFlavor).c_str()), 
  			     Form("%.1f<#eta<%.1f", etabins[iEtaBin-1], etabins[iEtaBin]),
  			     nptbins, ptbins);
      TH1F *h_width = new TH1F(Form("width_pt_eta%d%s", iEtaBin, flavors.at(iFlavor).c_str()),
  			     Form("%.1f<#eta<%.1f", etabins[iEtaBin-1], etabins[iEtaBin]),
  			     nptbins, ptbins);
      //TH1F *h_chi2  = new TH1F(Form("chi2_pt_eta%d%s", iEtaBin, flavors.at(iFlavor).c_str()), "", nptbins, ptbins);
    ...
    for (int iPtBin=1; iPtBin<nptbins+1;++iPtBin) {
      TH1F *h_mean  = new TH1F(Form("mean_eta_pt%d%s", iPtBin, flavors.at(iFlavor).c_str()),
  			     Form("%.1f<p_{T}^{ref}<%.1f", ptbins[iPtBin-1], ptbins[iPtBin]),
  			     netabins, etabins);
      TH1F *h_width = new TH1F(Form("width_eta_pt%d%s", iPtBin, flavors.at(iFlavor).c_str()),
  			     Form("%.1f<p_{T}^{ref}<%.1f", ptbins[iPtBin-1], ptbins[iPtBin]),
  			     netabins, etabins);
      //TH1F *h_chi2  = new TH1F(Form("chi2_eta_pt%d%s", iPtBin, flavors.at(iFlavor).c_str()), "", netabins, etabins);
  ...
  for (unsigned int iFlavor=0; iFlavor<flavors.size(); iFlavor++){
    for (int iEtaBin=1;iEtaBin<netabins+1;++iEtaBin) {
      for (int iPtBin=1;iPtBin<nptbins+1;++iPtBin) {
        ...
        TH1F *h = (TH1F*) ZJetBalance::DrawingHelperOk::GetObject(f, Form("DB_RefEtaBin%d_PtBin%d%s", iEtaBin, iPtBin, flavors.at(iFlavor).c_str()));
        h->SetStats(0);
        myFitter->FitAndDraw(h,fitMin);
        can->Print(pdf);
        
        TH1F *h_mean_pt  = (TH1F*)gROOT->FindObject(Form("mean_pt_eta%d%s", iEtaBin, flavors.at(iFlavor).c_str()));
        TH1F *h_width_pt = (TH1F*)gROOT->FindObject(Form("width_pt_eta%d%s", iEtaBin, flavors.at(iFlavor).c_str()));
        //TH1F *h_chi2_pt  = (TH1F*)gROOT->FindObject(Form("chi2_pt_eta%d%s", iEtaBin, flavors.at(iFlavor).c_str()));
        TH1F *h_mean_eta  = (TH1F*)gROOT->FindObject(Form("mean_eta_pt%d%s", iPtBin, flavors.at(iFlavor).c_str()));
        TH1F *h_width_eta = (TH1F*)gROOT->FindObject(Form("width_eta_pt%d%s", iPtBin, flavors.at(iFlavor).c_str()));
        //TH1F *h_chi2_eta  = (TH1F*)gROOT->FindObject(Form("chi2_eta_pt%d%s", iPtBin, flavors.at(iFlavor).c_str()));
